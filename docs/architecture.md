...
- OWL2（2024–2026）常规赛完成后的榜单以 `syntheticFinalStandings()` 为最终来源；2027+ 完成赛季统一经过 `b2ComputedStandingsRows()` 提供的 `__OWL_V34_STANDINGS_OVERRIDE`，再由 `standings34()` 暴露给赛季页、联盟榜单和季后赛资格。进行中的 2024–2026 榜单使用 `v741LiveStandings()`。所有来源都必须返回 `wins`、`losses` 和 `lp`，不能由各页面各自重新推算。
- 整季模拟在 Stage 2 结算后若存在 `v71AllStarPending`，必须先暂停并交给既有全明星 runtime；玩家处理全明星后再通过公共恢复协议继续，不能在批处理器内直接清掉 pending 标记。
- 赛季页、年度奖项页和赛季总结页的“进入季后赛”都允许从旧存档重建 `playoffState`，恢复入口集中在 `095-inline.js` 的 `openRecoveredPlayoffs()`，不在三个按钮上复制初始化逻辑；其中赛季页 CTA 通过文档级 capture listener 接管旧 `v71` 闭包绑定。

(function (root) {
  ...
  function renderBlockReport(state) {
    ...
    var triggers = report.projectedRelationTriggers && report.projectedRelationTriggers.length ? '<p>本区块可能触发：' + esc(report.projectedRelationTriggers.join('、')) + '</p>' : '';
    var header = '<p class="eyebrow">BLOCK REPORT · ' + report.stage + '-' + report.block + (report.key ? ' · 关键比赛' : '') + '</p><h3>' + report.wins + ' 胜 ' + report.losses + ' 负 · 平均评分 ' + Number(report.averageRating).toFixed(2) + '</h3><p class="muted">联盟预估第 ' + report.leagueRank + rankChange + ' · Stage 累计 ' + stage.wins + ' 胜 ' + stage.losses + ' 负 · 出场 ' + report.appearances + '/' + report.games + ' · 当前身份：' + esc(I.roleProfile(report.roleBefore).label) + ' · 预计：' + esc(I.roleProfile(report.projectedRoleAfter).label) + ' (' + (report.projectedRoleChange >= 0 ? '+' : '') + report.projectedRoleChange + ')</p>';
    var ledgerHtml = '<div class="impact-ledger"><strong>比赛影响账本（单位：百分点）</strong><div>队伍基础胜率 <b>' + Number(first.baseProbability).toFixed(1) + '%</b> · 无本轮备战 <b>' + Number(first.counterfactualProbability).toFixed(1) + '%</b> · 最终赛前胜率 <b>' + Number(first.finalProbability).toFixed(1) + '%</b></div><div>个人长期能力 ' + impactValue(first.playerAbilityPP) + ' · 版本适应 ' + impactValue(first.metaPP) + ' · 团队磨合 ' + impactValue(first.synergyPP) + '</div><div>比赛阅读 ' + impactValue(first.decisionPP) + ' · 本轮训练 ' + impactValue(first.actionPP) + ' · 当前状态 ' + impactValue(first.statePP) + ' · 随机波动 ' + impactValue(first.variancePP) + boundary + '</div><div>本区块个人出场贡献合计 ' + impactValue(totals.playerAbilityPP + totals.statePP + totals.actionPP) + ' · 训练直接作用合计 ' + impactValue(totals.actionPP) + '</div></div>';
    var feedbackHtml = '<div class="feedback-card"><strong>队伍反馈</strong><p>教练评价：' + esc(report.coachFeedback || '教练会继续观察你的表现。') + '</p><p>' + esc(report.teamFeedback || '确认比赛结果后生成队伍反馈。') + '</p><p>支援反馈：' + esc(report.supportFeedback || '支援关系暂无新的反馈。') + '</p>' + triggers + '</div>';
    var html = '<div class="block-report-card ' + (report.key ? 'key-report' : '') + '">' + header + '<div class="block-report-columns"><div class="report-main"><div class="match-list">' + rows + '</div></div><div class="report-side">' + ledgerHtml + feedbackHtml + '</div></div><button class="button primary" data-alpha-block-report>确认区块结算 →</button></div>';

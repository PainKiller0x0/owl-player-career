...
(function(){
  ...
  function b2BindStandingsTrigger(chip){if(!chip||chip.dataset.b2Standings)return;chip.dataset.b2Standings='1';chip.classList.add('b2-standings-trigger');chip.title='查看整个联盟战绩';chip.setAttribute('role','button');chip.setAttribute('tabindex','0');chip.addEventListener('click',b2OpenStandings);chip.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();b2OpenStandings();}});}

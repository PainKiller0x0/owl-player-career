...
(function(){
  ...
  if(typeof renderCareerTeam==='function'){const base=renderCareerTeam;renderCareerTeam=function(){const out=base.apply(this,arguments);b2DecorateAge();return out;};}
  b2DecorateAge();
  ...
  if(typeof renderMatch==='function'){const base=renderMatch;renderMatch=function(){const out=base.apply(this,arguments);b2DecorateMapPool();return out;};}
  ...
  if(typeof renderSeason==='function'){const base=renderSeason;renderSeason=function(){const out=base.apply(this,arguments);b2BindStandingsTrigger();return out;};}
  b2BindStandingsTrigger();

  /* 16 · 整季模拟性能：快速模式不再偷偷跑完整地图/选图/Ban内核 */
  const B2_oldWhole=window.__OWL_V18_FULL_SEASON;const B2_oldChance=typeof getRegularSeasonWinChance==='function'?getRegularSeasonWinChance:null;const B2_oldSilent=typeof v32SilentRegularGame==='function'?v32SilentRegularGame:null;let B2_wholeToken=0;const B2_rosterCache=new Map();const B2_heroFitCache=new WeakMap();
  ...
  function b2FastRegularGame(){
    if(b2Year()<2024&&B2_oldSilent)return B2_oldSilent();const idx=Number(seasonState.played||0),opp=seasonState.opponents?.[idx];if(!opp)return false;const our=careerState.starters||[],their=b2QuickRoster(opp),bonus=typeof currentCareerMatchBonus==='function'?currentCareerMatchBonus():0,venue=typeof regularVenueAt==='function'?regularVenueAt(idx):(seasonState.venues?.[idx]||'home'),chance=getRegularSeasonWinChance(our,their,bonus,venue),won=Math.random()<chance;seasonState.results[idx]=won?'win':'loss';seasonState.played=idx+1;if(won)seasonState.wins++;else seasonState.losses++;
  ...
  function b2ResolveStage(){const st=Number(seasonState.stageBreakPending||0);if(!st)return;window.__OWL_V16_SEASON_BATCHING=true;try{if(typeof stageQualified==='function'&&stageQualified(st))simulateStagePlayoff(st);else skipStageBreak(st);}finally{window.__OWL_V16_SEASON_BATCHING=false}seasonState.stageBreakPending=null;seasonState.v71LastMajorSummary=null;seasonState.v769TournamentResultPending=null;if(st===2&&seasonState.v71AllStarPending){try{if(typeof v71BuildAllStarResult==='function')v71BuildAllStarResult();}catch(_){}seasonState.v71AllStarPending=false;}}
  function b2StopWhole(msg=''){seasonState.b2WholeActive=false;seasonState.simulating=false;window.__OWL_B2_FAST_BATCH=false;window.__OWL_V16_SEASON_BATCHING=false;if(msg){const n=document.getElementById('seasonSimNote');if(n)n.textContent=msg}renderSeason();}
  function b2WholeSeason(){
    if(b2Year()>=2027&&typeof B2_oldWhole==='function')return B2_oldWhole();if(seasonState.b2WholeActive||seasonState.simulating||Number(seasonState.played)>=Number(seasonState.total))return;const wc=b2WorldCupDue();if(wc){window.__OWL_WORLD_CUP?.open?.();return;}seasonState.b2WholeActive=true;seasonState.simulating=true;window.__OWL_B2_FAST_BATCH=true;const token=++B2_wholeToken;const tick=()=>{if(token!==B2_wholeToken||!seasonState.b2WholeActive)return;let batch=0;while(batch++<28&&Number(seasonState.played)<Number(seasonState.total)){const before=Number(seasonState.played);window.__OWL_V16_SEASON_BATCHING=true;const ok=b2FastRegularGame();window.__OWL_V16_SEASON_BATCHING=false;if(!ok||Number(seasonState.played)<=before){b2StopWhole('模拟被当前流程节点暂停，请先处理当前节点。');return;}if(typeof markStageBreakIfNeeded==='function')markStageBreakIfNeeded();if(seasonState.stageBreakPending)b2ResolveStage();if(careerState.v800Trade?.pending){b2StopWhole('🔄 模拟在交易节点暂停。');return;}const world=b2WorldCupDue();if(world){seasonState.b2WholeActive=false;seasonState.simulating=false;window.__OWL_B2_FAST_BATCH=false;renderSeason();setTimeout(()=>window.__OWL_WORLD_CUP?.open?.(),40);return;}if(seasonState.currentEvent||seasonState.eventDue){seasonState.b2WholeActive=false;seasonState.simulating=false;window.__OWL_B2_FAST_BATCH=false;seasonState.resumeWholeAfterEvent=true;renderSeason();setTimeout(openScheduledSeasonEvent,50);return;}}
    try{window.__OWL_PUBLIC_BETA?.autosave?.('whole-season-batch',120)}catch(_){}const note=document.getElementById('seasonSimNote');if(note)note.textContent=`⏳ 正在轻量模拟：${seasonState.played}/${seasonState.total} · ${seasonState.wins}胜${seasonState.losses}负`;renderSeason();if(Number(seasonState.played)>=Number(seasonState.total)){b2StopWhole(`✓ 常规赛与阶段赛事已完成：${seasonState.wins}胜${seasonState.losses}负。年终季后赛保留。`);return;}setTimeout(tick,0);};renderSeason();setTimeout(tick,0);
  }
  ...
  if(typeof renderRetirementScreen==='function'){const base=renderRetirementScreen;renderRetirementScreen=function(){const out=base.apply(this,arguments);b2DecorateRetirement();return out;};}
  if(typeof renderRetiredCareerResume==='function'){const base=renderRetiredCareerResume;renderRetiredCareerResume=function(){const out=base.apply(this,arguments);b2DecorateRetirement();return out;};}

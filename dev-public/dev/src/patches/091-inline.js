...
(function(){
  ...
  function b2Conf(team){try{if(typeof window.__OWL_V771_REGION==='function')return window.__OWL_V771_REGION(team);if(b2Year()>=2024&&typeof v71Conference==='function')return v71Conference(team);if(typeof v762Region==='function')return v762Region(team,b2Year());}catch(_){}return team?.conference||((team?.division==='Atlantic')?'East':team?.division==='Pacific'?'West':'GLOBAL');}
  ...
  function b2RenderStandings(){const o=b2EnsureStandingsModal(),rows=b2StandingsRows(),hasRegions=rows.some(r=>r.region==='East'||r.region==='West');if(!hasRegions)b2StandingsTab='ALL';const tabs=[['ALL','总战绩'],...(hasRegions?[['East','东部战绩'],['West','西部战绩']]:[])];o.querySelector('#b2StandingsTitle').textContent=`${b2Year()} 赛季 · 联盟战绩`;o.querySelector('#b2StandingsSub').textContent=`已进行 ${Number(seasonState.played||0)} / ${Number(seasonState.total||0)} 场 · 点击年份可随时回来查看`;o.querySelector('#b2StandingsTabs').innerHTML=tabs.map(([k,l])=>`<button class="b2-standings-tab ${b2StandingsTab===k?'active':''}" data-b2-tab="${k}">${l}</button>`).join('');const shown=(b2StandingsTab==='ALL'?rows:rows.filter(r=>r.region===b2StandingsTab));const lp=shown.some(r=>Number(r.lp||0)!==Number(r.wins||0));o.querySelector('#b2StandingsBody').innerHTML=`<table class="b2-standings-table"><thead><tr><th>#</th><th>队伍</th><th>胜</th><th>负</th><th>净胜图</th>${lp?'<th>LP</th>':''}</tr></thead><tbody>${shown.map((r,i)=>{const regionKey=r.region==='East'?'east':r.region==='West'?'west':'global',regionLabel=r.region==='East'?'东部':r.region==='West'?'西部':'全联盟';return `<tr class="${r.isUser?'user':''}"><td class="b2-rank">${i+1}</td><td><div class="b2-team-cell">${r.team?.logo?`<img src="${b2Esc(r.team.logo)}" onerror="this.style.display='none'">`:''}<div><b>${b2Esc(b2TeamName(r.team))}</b><small>${r.isUser?'你的队伍 · ':''}<span class="b2-region-badge ${regionKey}">${regionLabel}</span></small></div></div></td><td>${r.wins}</td><td>${r.losses}</td><td>${Number(r.mapDiff||0)>0?'+':''}${Number(r.mapDiff||0)}</td>${lp?`<td>${Number(r.lp??r.wins)}</td>`:''}</tr>`;}).join('')}</tbody></table>`;o.querySelectorAll('[data-b2-tab]').forEach(b=>b.onclick=()=>{b2StandingsTab=b.dataset.b2Tab;b2RenderStandings()});}
  function b2OpenStandings(){b2StandingsTab='ALL';b2RenderStandings();b2EnsureStandingsModal().classList.remove('ui-hidden');}
  function b2BindStandingsTrigger(chip){if(!chip||chip.dataset.b2Standings)return;chip.dataset.b2Standings='1';chip.classList.add('b2-standings-trigger');chip.title='查看整个联盟战绩';chip.setAttribute('role','button');chip.setAttribute('tabindex','0');}
  function b2BindStandingsTriggers(){b2BindStandingsTrigger(document.getElementById('seasonYearChip'));b2BindStandingsTrigger(document.getElementById('careerSeasonChip'));}
  window.__OWL_RUNTIME?.render?.register('renderSeason','b2-standings-trigger',b2BindStandingsTriggers);
  window.__OWL_RUNTIME?.render?.register('renderCareerTeam','b2-standings-trigger',b2BindStandingsTriggers);
  if(!window.__OWL_B2_STANDINGS_DELEGATED){
    const findStandingsChip=target=>target instanceof Element?target.closest('#seasonYearChip,#careerSeasonChip'):null;
    document.addEventListener('click',event=>{if(findStandingsChip(event.target))b2OpenStandings();},true);
    document.addEventListener('keydown',event=>{if((event.key==='Enter'||event.key===' ')&&findStandingsChip(event.target)){event.preventDefault();b2OpenStandings();}},true);
    window.__OWL_B2_STANDINGS_DELEGATED=true;
  }
  b2BindStandingsTriggers();
